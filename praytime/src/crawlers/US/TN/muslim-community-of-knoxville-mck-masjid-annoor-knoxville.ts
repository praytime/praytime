...
const run = async () => {
  const $ = await util.load(
    "https://muslimknoxville.org/services/prayers-and-jumuah/",
  );

  const a = util.mapToText($, ".dpt_jamah");
  const j = util
    .mapToText($, '.elementor-icon-list-text:contains("Khut")')
...
export const crawler: CrawlerModule = {
  name: "US/TN/muslim-community-of-knoxville-mck-masjid-annoor-knoxville",
  ids,
  run,
};

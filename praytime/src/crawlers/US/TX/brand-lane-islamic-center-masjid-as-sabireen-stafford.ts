...
const run = async () => {
  ...
  const a = util
    .mapToText($, ".execphpwidget td:last-child")
  ...
  util.setJumaTimes(
    ids[0],
    a.filter((t) => t.match(/Khutbah/)).map(util.extractTimeAmPm),
  );
...
export const crawler: CrawlerModule = {
  name: "US/TX/brand-lane-islamic-center-masjid-as-sabireen-stafford",
  ids,
  run,
};

...
const run = async () => {
  const $ = await util.load(
    "https://us.mohid.co/wi/milwaukee/alhuda/masjid/widget/api/index/?m=prayertimings",
  );
  ...
  util.setJumaTimes(
    ids[0],
    j.filter((t) => t.match(/gfield.*khutba/i)).map(util.extractTimeAmPm),
  );
  util.setJumaTimes(
    ids[1],
    j.filter((t) => t.match(/smke.*khutba/i)).map(util.extractTimeAmPm),
  );
...
export const crawler: CrawlerModule = {
  name: "US/WI/masjid-al-huda-greenfield",
  ids,
  run,
};

...
const run = async () => {
  ...
  ids[0].maghribIqama = $("#ctl00__PrayerTimetable1_lblMaghribStart")
    .text()
    .trim();
  ids[0].ishaIqama = $("span[id=ctl00__PrayerTimetable1_lblIshaStart]")
    .eq(0)
    .text()
    .trim();
  ids[0].juma1 = $("span[id=ctl00__PrayerTimetable1_lblIshaStart]")
    .eq(1)
    .text()
    .trim();
...
export const crawler: CrawlerModule = {
  name: "US/TX/makkah-masjid-garland",
  ids,
  run,
};

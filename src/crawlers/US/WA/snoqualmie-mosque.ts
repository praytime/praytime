...
const run = async () => {
  ...
  try {
    ...
    await page.goto("http://www.snoqualmiemosque.org/prayers.html", {
      waitUntil: "networkidle0",
    });

    const t = await page.$$eval('tr[style*="white"] td[style*="bold"]', (es) =>
      es.map((e) => e.textContent.trim()),
    );
    util.setIqamaTimes(ids[0], t);

    const j = await page.$$eval("span", (es) =>
      es

...
exports.run = async () => {
  const date = new Date()
  const response = await axios.get('https://us.mohid.co/il/nwcs/icw/masjid/widget/api/index/?m=prayertimings')
  ...
  results[0].juma1 = $('#jummah > div > ul > li:nth-child(1) > div.prayer_iqama_div').text().trim()

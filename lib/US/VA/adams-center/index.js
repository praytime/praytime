...
exports.run = async () => {
  ...
  try {
    ...
    // page.on('frameattached', (frame) => { console.error('frameattached ', frame.url()) })
    page.on('framedetached', (frame) => { console.error('framedetached ', frame.url()) })
    page.on('framenavigated', (frame) => { console.error('framenavigated ', frame.url()) })
    ...
    const fn = async (frame) => {
      for (const child of frame.childFrames()) {
        console.error(`${frame.url()}: ${child.url()}`)
        if (child.url().includes('masjidbox.com')) {
          try {
            await child.waitForSelector('div.iqamah div.time')
            const t = await child.$$eval('div.iqamah div.time', (divs) => divs.map((div) => div.textContent.trim()))
            ids.forEach((r) => {
              if (!/Juma/.test(r.name)) {

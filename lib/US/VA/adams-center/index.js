...
exports.run = async () => {
  const date = new Date()
  // %-m removes leading 0 from %m https://stackoverflow.com/a/2073189
  const month = us(date, results[0].timeZoneId, '%-m')
  const day = us(date, results[0].timeZoneId, '%-d')

  const response = await axios.request({
    url: 'https://www.adamscenter.org//_api/wix-code-public-dispatcher/siteview/wix/data-web.jsw/find.ajax',
    method: 'post',
    headers: { 'Content-Type': 'application/json' },
    params: {
      gridAppId: '05d4bac5-c9c8-42ae-a58f-87e54e43ecf1',
      instance: 'wixcode-pub.f57452ff62fc0e9f844b3f51c18ee9dc880f4f00.',
      viewMode: 'site'
    },
    data: '["PrayerTimes",{"$and":[{"month":' + month + '},{"day":' + day + '}]},null,0,50,null,[]]'
  })

  const p = response.data.result.items[0]

  results.forEach((r) => {
    r.crawlTime = date
    if (!/Juma/.test(r.name)) {
      r.fajrIqama = p.fajrIqama
      r.zuhrIqama = p.dhuhurIqama
      r.asrIqama = p.asrIqama
      r.maghribIqama = p.maghribIqama
      r.ishaIqama = p.ishaIqama
    }

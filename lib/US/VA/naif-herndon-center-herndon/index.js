...
exports.run = async () => {
  ...
  const a = util.mapToText($, '.prayer-timings-table td:last-child')
    .filter(util.matchTimeAmPm)
  util.setIqamaTimes(ids[0], a)
  util.setIqamaTimes(ids[1], a)

  util.setJumaTimes(ids[0], util.mapToText($, '.prayer-timings-table td:contains("Herndon") + td'))
  util.setJumaTimes(ids[1], util.mapToText($, '.prayer-timings-table td:contains("Manassas") + td'))

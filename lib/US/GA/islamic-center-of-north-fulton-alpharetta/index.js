...
exports.run = async () => {
  ...
  //   }
  // }
  const month = util.strftime('%m', ids[0].timeZoneId)
  const day = util.strftime('%d', ids[0].timeZoneId)
  const [d, $] = await Promise.all([
    util.loadJson(`https://masjiddashboard.com/api/prayer/companyId/6046bc4b37e1105575a61ef0/month/${month}/day/${day}`),
    util.load(ids[0].url)
  ])
  ...
  const j = util.mapToText($, '.textwidget b')

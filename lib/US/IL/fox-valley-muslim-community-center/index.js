...
exports.run = async () => {
  ...
  ids[0].fajrIqama = text.match(/Fajr\s*:\s*(\S+)/)[1]
  ids[0].zuhrIqama = text.match(/Duhr\s*:\s*(\S+)/)[1]
  ids[0].asrIqama = text.match(/Asr\s*:\s*(\S+)/)[1]
  ids[0].maghribIqama = text.match(/Maghrib\s*:\s*(\S+)/)[1]
  ids[0].ishaIqama = text.match(/Isha\s*:\s*(\S+)/)[1]
  ids[0].juma1 = text.match(/Jumah\s*:\s*(\S+)/)[1]

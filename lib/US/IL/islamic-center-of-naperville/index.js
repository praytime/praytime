...
exports.run = async () => {
  ...
  $('tr:contains("Iqama Timing")').remove()

  util.setTimes(ids[0], util.mapToText($, '[aria-labelledby="fusion-tab-ogdenmasjid"] td:not([colspan]):last-child'))
  util.setTimes(ids[1], util.mapToText($, '[aria-labelledby="fusion-tab-olesenmasjid"] td:not([colspan]):last-child'))
  util.setTimes(ids[2], util.mapToText($, '[aria-labelledby="fusion-tab-75thmasjid"] td:not([colspan]):last-child'))

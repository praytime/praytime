...
exports.run = async () => {
  const data = await util.loadJson('https://masjidal.com/api/v1/time?masjid_id=pQKMn3LB')

  if (data.status === 'success') {
    const iqama = data.data.iqama
    util.setIqamaTimesAll(ids, [
      iqama.fajr,
      iqama.zuhr,
      iqama.asr,
      iqama.maghrib,
    ...
    util.setJumaTimesAll(ids, [
      iqama.jummah1
    ])
    // olesen has single juma only
    ids[0].juma2 = iqama.jummah2
    ids[0].juma3 = iqama.jummah3
    ids[2].juma2 = iqama.jummah2
    ids[2].juma3 = iqama.jummah3
  }

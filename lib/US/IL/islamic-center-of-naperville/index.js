...
exports.run = async () => {
  ...
  // 248th:  https://masjidal.com/widget/monthly/?masjid_id=JAmlb9LR&label_athan=ATHAN&label_iqamah=IQAMAH
  const dd = await Promise.all([
    ...
    util.loadJson('https://masjidal.com/api/v1/time?masjid_id=JAmlb9LR')
  ])
  ...
  dd.forEach((d, i) => {
    if (d.status === 'success') {
      ...
      util.setJumaTimes(ids[i], [iqama.jummah1, iqama.jummah2, iqama.jummah3])
    }
  })

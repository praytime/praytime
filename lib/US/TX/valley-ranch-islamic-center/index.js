...
//
exports.run = async () => {
  const response = await axios.get('https://api.masjidapps.com/masjid/publicPrayerTimes/MQ2/NzY1ZjcxZmQtZjE0NS00OGFjLTljYTgtMjBiYmRlYjdkZGRj0')
  const prayerTimes = response.data
  ids[0].fajrIqama = prayerTimes.fajrIqamah
  ids[0].zuhrIqama = prayerTimes.duhrIqamah
  ids[0].asrIqama = prayerTimes.asrIqamah
  ids[0].maghribIqama = prayerTimes.maghribIqamah
  ids[0].ishaIqama = prayerTimes.ishaIqamah
  ids[0].juma1 = prayerTimes.firstJummahAdhan
  ids[0].juma2 = prayerTimes.secondJummahAdhan

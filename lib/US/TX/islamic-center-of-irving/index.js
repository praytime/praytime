...
exports.run = async () => {
  ...
  const a = util.mapToText($, 'table.dptTimetable th:contains("Iqamah") ~ td')
  // On Juma, zuhr is replaced
  if (day === '5') {
    a.splice(1, 0, 'check website')
  }
  util.setIqamaTimes(ids[0], a)
  const j = util.mapToText($, 'table.dptTimetable th:contains("Jumuah") + td')[0]
    .split(' | ')
    .map((t) => t.trim())

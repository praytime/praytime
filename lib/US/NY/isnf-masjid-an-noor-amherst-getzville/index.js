...
exports.run = async () => {
  const $ = await util.load('https://www.isnf.org/services-3')

  // 'p.font_5': 'Jumuah Time', 'Dhuhr Time', 'Maghreb Time', 'Fajr Time', 'Asr Time', 'Isha Time'
  const aa = [
    util.mapToText($, 'p', $('p.font_5:contains("Fajr")').closest('div').next()),
    util.mapToText($, 'p', $('p.font_5:contains("Dhuhr")').closest('div').next()),
    util.mapToText($, 'p', $('p.font_5:contains("Asr")').closest('div').next()),
    util.mapToText($, 'p', $('p.font_5:contains("Maghreb")').closest('div').next()),
    util.mapToText($, 'p', $('p.font_5:contains("Isha")').closest('div').next()),
    util.mapToText($, 'p', $('p.font_5:contains("Jumuah")').closest('div').next())
  ].flat()
  // [
  //   [
  ...
  util.setIqamaTimes(ids[0], aa
    .filter(a => a.match(/Noor/))
    .map(util.extractTimeAmPm))
  util.setIqamaTimes(ids[1], aa
    .filter(a => a.match(/Taqwa/))
    .map(util.extractTimeAmPm))

  util.setJumaTimes(ids[0], aa
    .filter(a => a.match(/jumuah/))
    .map(util.extractTimeAmPm))

  util.setJumaTimes(ids[1], aa
    .filter(a => a.match(/both.*jumuah/i))
    .map(util.extractTimeAmPm))
